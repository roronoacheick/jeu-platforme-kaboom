...
const {
  loadSprite, scene, go, add, text, pos,
  sprite, rect, area, body, scale, anchor, color,
  onKeyPress, onKeyDown, onCollide, onUpdate,
  wait, lifespan, destroy, fixed, width, height,
  vec2, outline, dt
} = k;
...
scene("level1", () => {
  ...
  const livesLabel = add([
    ...
    pos(width()-80,10), fixed(), color(255,255,255)
  ]);
  ...
  onCollide("player","enemy",(p,e) => {
    ...
    if (vies <= 0) {
      ...
      // Respawn après 1s pour toujours avoir un ennemi
      wait(1, () => spawnEnemies(1, player, "enemy1"));
    }
  });
});
...
scene("level2", () => {
  ...
  onCollide("player","enemy",(p,e) => {
    ...
    if (vies <= 0) {
      ...
      // Respawn 1 seul ennemi après 1s
      wait(1, () => spawnEnemies(1, player, "enemy2"));
    }
  });
});
...
scene("boss", () => {
  ...
  // Le boss reste immobile, envoie 3 ennemis de droite à vitesse 200
  spawnEnemies(3, player, "enemy3", true, 200);
  ...
  const timerText = add([
    ...
    pos(width()/2,30), anchor("center"), fixed(), color(255,255,255)
  ]);
  ...
  onKeyPress("r",() => {
    ...
    laser.onUpdate(() => {
      ...
      if (laser.pos.x < 0 || laser.pos.x > width() || laser.pos.y < 0 || laser.pos.y > height())
        destroy(laser);
    });
  });
  ...
  // Ennemi touche joueur → mort immédiate
